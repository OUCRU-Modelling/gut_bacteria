...
v           <- c(1,0.5,0.5)          ### flow velocity of Food, Antibiotic, Bacteria and Mutant respectively
D           <- c(05.0,0.2,0.2)       ### Diffusion coefficient of Food, Antibiotic, Bacteria and Mutant respectively
...
delta_wo    <- (alpha1 - (v[1]^2)/(4*D[1]))*(1/alpha2)        ### Possible wash out limit
delta_max   <- delta_wo + 2*10^-1
...
Sol_system  <- function(t, Y, parms) {
  ...
  dA        <- -(delta_max/beta)*B*(A)/(A+A_50) + tran.1D(C = A   , D = D[1] , flux.up = 1   , flux.down = NULL, v = v[1], dx = xgrid)$dC
  dB        <- r*B*Food/(k+Food) + -delta_max*B*(A)/(A+A_50) + tran.1D(C = B , D = D[1], flux.up = 0 , flux.down = NULL, v = v[1], dx = xgrid)$dC                                         ### tran1D to describe divection diffusion equation
...
###### Draw the full non-mutant profile to text the washout limit for delta max ############
