...
library(deSolve)
L         <- 6
v         <- seq(0.05, 10, len=30)
k         <- 0.1
F_in      <- 1/v
D         <- seq(0.05, 50, len=30)        # Assigning the parameters values
r         <- 0.42
alpha     <- 6.13*(10^8)
...
while(i<=length(v)){
  for(j in 1:length(D)){
    ...
    #j <- j+1
    conc_profile[i,j] <- (out[length(times),2]-out[length(times),N+1])/F_in[i]
  }
    i <- i+1
}
heatmap(t(conc_profile), Rowv=NA, Colv=NA, col = hcl.colors(61))