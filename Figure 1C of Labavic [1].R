...
alpha         <- 6.13*(10^8)
k             <- 0.1
v             <- c(seq(0.05, 0.1, len=19), seq(0.105, 0.2, len=15), seq(0.201, 0.5, len = 14), seq(0.505, 1, len=8), seq(1.1, 2.6, len =5))     ### v and D scale have diffrent step sizes in it (Focus on Figure C, you can see the different leaps for diffrent interval of values)
F_in          <- 1/v
...
load("~/Master Thesis/gut_bacteria/Data for 64x61 matrix for Fig 1C Labavic.RData")

par(mar = c(4.8, 4.2, 5.5, 4.9) + 0.07)
x <- c(v[1]/2, v)
y <- c(D[1]/2, D)
x[which(x==1)[2]]<- 1.001 # little trick because twice the value 1
image.plot(t(conc_profile), x=log10(x), y=log10(y),col = NULL, axes=FALSE)
contour(z = t(conc_profile),x=log10(x[2:length(x)]), y=log10(y[2:length(y)]), add=TRUE)
